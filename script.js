...
let playerAngleMoved2;
let accelerate = false; // Is the player accelerating
let decelerate = false; // Is the player decelerating
let accelerate2 = false; // Is the player accelerating
let decelerate2 = false; // Is the player decelerating
...
function movePlayerCar(timeDelta) {
  ...
  const playerSpeed2 = getPlayerSpeed2();

  playerAngleMoved -= playerSpeed * timeDelta;
  playerAngleMoved2 -= playerSpeed2 * timeDelta;

  const totalPlayerAngle1 = playerAngleInitial + playerAngleMoved;
  const totalPlayerAngle2 = 0 + playerAngleMoved2;

  const playerX1 = Math.cos(totalPlayerAngle1) * trackRadius - arcCenterX;
  const playerY1 = Math.sin(totalPlayerAngle1) * trackRadius;
  const playerX2 = Math.cos(totalPlayerAngle2) * trackRadius + arcCenterX;
  const playerY2 = Math.sin(totalPlayerAngle2) * trackRadius;

  playerCar.position.x = playerX1;
  playerCar.position.y = playerY1;
  playerCar.rotation.z = totalPlayerAngle1 - Math.PI / 2;

  playerCar2.position.x = playerX2;
  playerCar2.position.y = playerY2;
  playerCar2.rotation.z = totalPlayerAngle2 - Math.PI / 2;
}
...
function getPlayerSpeed2() {
  if (accelerate2) return speed2 * 2;
  if (decelerate2) return speed2 * 0.5;
  return speed2;
}
...
function hitDetection() {
  ...
  if (
    getDistance(playerHitZone1, player2HitZone1) < 40 ||
    getDistance(playerHitZone1, player2HitZone2) < 40 ||
    getDistance(playerHitZone2, player2HitZone1) < 40
  )
    resolveHit();
}
